...
class FamilyNetwork {
  constructor(store, onChange, port = PORT, appVersion = '0.0.0') { this.store=store; this.onChange=onChange; this.port=port; this.appVersion=appVersion; this.pairFailures=new Map(); this.server=null; this.timer=null; this.running=false; }
    ...
    s.network.deviceId ||= id(); s.network.deviceName ||= os.hostname(); s.network.peers ||= {}; s.network.remoteItems ||= {};s.network.managedConfigs||={};s.network.itemOverrides||={};s.network.hostKnownAssetIds||=[];s.network.managementEnabled=Boolean(s.network.managementEnabled);s.network.pendingManagerActions||=[];s.network.appliedManagerActions||={};s.network.auditLog||=[];s.network.managerDevices||=[];s.network.managerAudit||=[];s.network.memberRole||='member';
    return s.network;
  }
    ...
    this.server=http.createServer(async(req,res)=>{
      try {
        ...
        if(req.url==='/sync') {
          ...
          this.store.save();this.onChange();return json(res,200,{peers:this.publicPeers(),items:{[n.deviceId]:hostItems,...remoteForSync},managedConfig,ackedActionIds,managerDevices:peer.role==='co-manager'?this.managementView():[],managerAudit:peer.role==='co-manager'?this.auditView():[],knownAssetIds:Object.keys(this.store.state.soundAssets),assets});
        }
  ...
  async sync(){
    ...
    try{
      ...
      const result=await request(n.hostIp,'/sync',{deviceId:n.deviceId,deviceName:n.deviceName,appVersion:this.appVersion,managementEnabled:managed,settings:managedSettings(this.store.state.settings),usage:this.store.state.usage,lastAppliedResetId:n.lastAppliedResetId,managerActions:n.pendingManagerActions,reminders:managed?this.store.state.reminders:this.store.state.reminders.filter(x=>x.shared),tasks:managed?this.store.state.tasks:this.store.state.tasks.filter(x=>x.shared),completions:this.store.state.completions,haveAssetIds:Object.keys(localAssets),assets},{'x-family-token':n.token},this.port);
      ...
      const acked=new Set(result.ackedActionIds||[]);n.pendingManagerActions=n.pendingManagerActions.filter(x=>!acked.has(x.id));if(n.memberRole==='co-manager'){n.managerDevices=result.managerDevices||[];n.managerAudit=result.managerAudit||[];}else{n.managerDevices=[];n.managerAudit=[];}
