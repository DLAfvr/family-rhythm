...
class FamilyNetwork {
    ...
    this.server=http.createServer(async(req,res)=>{
      try {
        ...
        if(req.url==='/sync') {
          ...
          n.remoteItems[data.deviceId]={reminders:data.reminders||[],tasks:data.tasks||[],completions:data.completions||[],settings:managedSettings(data.settings),usage:data.usage||{},lastAppliedResetId:data.lastAppliedResetId||null,parentLockConfigured:Boolean(data.parentLockConfigured),lastAppliedParentLockRequestId:data.lastAppliedParentLockRequestId||null,receivedAt:new Date().toISOString()};
  ...
  async sync(){
    ...
    try{
      ...
      const result=await request(n.hostIp,'/sync',{deviceId:n.deviceId,deviceName:n.deviceName,appVersion:this.appVersion,managementEnabled:managed,settings:managedSettings(this.store.state.settings),usage:this.store.state.usage,lastAppliedResetId:n.lastAppliedResetId,parentLockConfigured:Boolean(this.store.state.settings.parentPassword),lastAppliedParentLockRequestId:n.lastAppliedParentLockRequestId,managerActions:n.pendingManagerActions,securityEvents:n.pendingSecurityEvents,reminders:managed?this.store.state.reminders:this.store.state.reminders.filter(x=>x.shared),tasks:managed?this.store.state.tasks:this.store.state.tasks.filter(x=>x.shared),completions:this.store.state.completions,haveAssetIds:Object.keys(localAssets),assets},{'x-family-token':n.token},this.port);
      if(n.hostOfflineAlerted)this.notifications.push({type:'device-online',deviceId:n.hostDevice?.deviceId,deviceName:n.hostDevice?.name||'主要電腦',body:'主要電腦已恢復連線'});n.hostOfflineSince=null;n.hostOfflineAlerted=false;n.online=true;n.lastSync=new Date().toISOString();n.peers=Object.fromEntries((result.peers||[]).map(x=>[x.deviceId,x]));n.memberRole=n.peers[n.deviceId]?.role||'member';n.remoteItems=result.items||{};n.hostKnownAssetIds=result.knownAssetIds||n.hostKnownAssetIds||[];
      ...
      if(managed&&result.managedConfig){Object.assign(this.store.state.settings,managedSettings(result.managedConfig.settings));this.store.state.reminders=result.managedConfig.reminders||[];this.store.state.tasks=result.managedConfig.tasks||[];n.appliedManagedVersion=result.managedConfig.version;const reset=result.managedConfig.resetUsageRequest;if(reset?.id&&reset.id!==n.lastAppliedResetId){this.store.state.usage[`me:${localDateKey()}`]=0;n.lastAppliedResetId=reset.id;n.lastUsageResetAt=new Date().toISOString();}const lock=result.managedConfig.parentLockRequest;if(lock?.id&&lock.id!==n.lastAppliedParentLockRequestId)n.parentLockRequest=lock;}
